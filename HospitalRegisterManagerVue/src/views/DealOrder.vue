  ...
  <div>
    ...
    <el-dialog title="病因编写" :visible.sync="reasonFormVisible">
      ...
      <div slot="footer" class="dialog-footer">
        <el-button @click="reasonFormVisible = false" style="font-size: 18px;">
          取 消</el-button
        >
...
export default {
  ...
  methods: {
    ...
    dataPackage() {
      ...
      return { oId, oRecord, oDrug, oItem, oMaterial, oTotalPrice };
    },
    //点击病因保存按钮
    holdReason() {
      ...
      for (let i = 0; i < this.materialBuyData.length; i++) {
        if (this.materialBuyData[i].mId === mId) {
          for (let j = 0; j < this.materialData.length; j++) {
            if (this.materialData[j].mId === mId)
              this.materialData[j].mNumber += 100;
          }
          this.materialBuyData[i].mNum -= 100;
          this.materialBuyData[i].mSum =
            this.materialBuyData[i].mPrice * this.materialBuyData[i].drNum;
          this.materialTotalPrice -= this.materialBuyData[i].drPrice; //药物价格总计
          if (this.materialBuyData[i].mNum === 0)
