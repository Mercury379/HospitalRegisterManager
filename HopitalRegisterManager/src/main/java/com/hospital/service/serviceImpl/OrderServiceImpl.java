...
public class OrderServiceImpl implements OrderService {
    ...
    public Boolean addOrder(Orders order, String arId){
        ...
        String time = String.valueOf(order.getORegisterTime()).substring(11,22);
        synchronized (this) {
        ...
        jedis.close();
        //redis结束
        order.setOId(String.valueOf(RandomUtil.randomOid(Integer.parseInt(order.getPId()))));
        order.setOStatus(0);
        order.setOPriceStatus(0);
        order.setORegisterTime(order.getORegisterTime());
        order.setORegisterTime((order.getORegisterTime().substring(0,22)));
    ...
    public Boolean updateOrder(Orders orders) {
        orders.setOStatus(1);
        //orders.setOPayTime(java.sql.Date.valueOf(TodayUtil.getToday()));
        QueryWrapper<Orders> wrapper = new QueryWrapper<>();
    ...
    public Boolean updatePrice(int oId){
        ...
        wrapper.eq("o_id", oId).set("o_price_status", 1).set("o_total_price", 0.00);
    ...
    public HashMap<String, Object> findOrderFinish(int pageNumber, int size, String query, int dId){
        Page<Orders> page = new Page<>(pageNumber, size);
        QueryWrapper<Orders> wrapper = new QueryWrapper<>();
        wrapper.like("p_id", query).eq("d_id", dId).orderByDesc("o_register_time").eq("o_status", 1);
        IPage<Orders> iPage = this.orderMapper.selectPage(page, wrapper);
        HashMap<String, Object> hashMap = new HashMap<>();
        hashMap.put("total", iPage.getTotal());       //总条数
        hashMap.put("pages", iPage.getPages());       //总页数
    ...
    public HashMap<String, Object> findOrderByDid(int pageNumber, int size, String query, int dId){
        Page<Orders> page = new Page<>(pageNumber, size);
        List<Orders> orders=this.orderMapper.findOrderByDid(dId);
        QueryWrapper<Orders> wrapper = new QueryWrapper<>();
        wrapper.like("p_id", query).eq("d_id", dId).orderByDesc("o_register_time");
        IPage<Orders> iPage = this.orderMapper.selectPage(page, wrapper);
        HashMap<String, Object> hashMap = new HashMap<>();
        hashMap.put("total", iPage.getTotal());       //总条数
        hashMap.put("pages", iPage.getPages());       //总页数
        hashMap.put("pageNumber", iPage.getCurrent());//当前页
        hashMap.put("orders", orders); //查询到的记录
        return hashMap;
    }
    /**
     * 统计今天排队人数
     */
    public int orderPeople(String pId){
        return this.orderMapper.orderPeople(pId);
    }
    /**
     * 签到
     */
    public Boolean qiandao(String pId){
        this.orderMapper.qiandao(pId);
        return true;
    }
    /**
     * 统计今天某个医生挂号人数
